...
const RecruiterProfile = () => {
  ...
  const [metrics, setMetrics] = useState({
    totalJobPostings: { value: 0, icon: <FaBriefcase className="text-blue-500 text-4xl" /> },
    candidatesAssessed: { value: 0, icon: <FaClipboardCheck className="text-green-500 text-4xl" /> },
    hiredCandidates: { value: 0, icon: <FaUserCheck className="text-purple-500 text-4xl" /> },
    avgApplicantsPerJob: { value: 0, icon: <FaUsers className="text-yellow-500 text-4xl" /> },
    interviewSuccessRate: { value: "0%", icon: <FaChartLine className="text-indigo-500 text-4xl" /> },
    avgTimeToHire: { value: "0 days", icon: <FaClock className="text-orange-500 text-4xl" /> },
    pendingInterviews: { value: 0, icon: <FaUserTie className="text-red-500 text-4xl" /> },
    rejectedCandidates: { value: 0, icon: <FaTimes className="text-gray-500 text-4xl" /> },
    offersAccepted: { value: 0, icon: <FaCheckCircle className="text-green-500 text-4xl" /> },
    totalAssessmentsSent: { value: 0, icon: <FaFileAlt className="text-blue-600 text-4xl" /> },
  });

  useEffect(() => {
    const fetchRecruiterData = async () => {
      try {
        const response = await fetch("http://localhost:5001/recruiter/67cde8f3abd6fbeabd40feef");
        const data = await response.json();

        if (!data) throw new Error("Invalid recruiter data");

        setJobPostings(data.jobPostings || []);
        setFavoriteStudents(data.favoriteStudents || []);
        
        setMetrics({
          totalJobPostings: { value: data.metrics.totalJobPostings || 0, icon: <FaBriefcase className="text-blue-500 text-4xl" /> },
          candidatesAssessed: { value: data.metrics.candidatesAssessed || 0, icon: <FaClipboardCheck className="text-green-500 text-4xl" /> },
          hiredCandidates: { value: data.metrics.hiredCandidates || 0, icon: <FaUserCheck className="text-purple-500 text-4xl" /> },
          avgApplicantsPerJob: { value: data.metrics.avgApplicantsPerJob || 0, icon: <FaUsers className="text-yellow-500 text-4xl" /> },
          interviewSuccessRate: { value: data.metrics.interviewSuccessRate || "0%", icon: <FaChartLine className="text-indigo-500 text-4xl" /> },
          avgTimeToHire: { value: data.metrics.avgTimeToHire || "0 days", icon: <FaClock className="text-orange-500 text-4xl" /> },
          pendingInterviews: { value: data.metrics.pendingInterviews || 0, icon: <FaUserTie className="text-red-500 text-4xl" /> },
          rejectedCandidates: { value: data.metrics.rejectedCandidates || 0, icon: <FaTimes className="text-gray-500 text-4xl" /> },
          offersAccepted: { value: data.metrics.offersAccepted || 0, icon: <FaCheckCircle className="text-green-500 text-4xl" /> },
          totalAssessmentsSent: { value: data.metrics.totalAssessmentsSent || 0, icon: <FaFileAlt className="text-blue-600 text-4xl" /> },
        });
